...
local function meme_squares(size, fg)
    local img = cairo.ImageSurface(cairo.Format.ARGB32, 6 * size, 2 * size)
    local cr = cairo.Context(img)
    cr:set_source(gears_color(fg))
    cr:set_line_width(size/4)
    cr:rectangle(4.1 * size, size, size, size)
    cr:fill()
    cr:stroke()
    return img
end
...
local function meme_squares_unsel(size, fg)
    local img = cairo.ImageSurface(cairo.Format.ARGB32, 6 * size, 2 * size)
    local cr = cairo.Context(img)
    cr:set_source(gears_color(fg))
    cr:set_line_width(size/4)
...
theme.font          = "Cousine Nerd Font 11"
theme.transparency = "50"

theme.bg_normal     = wal.c00
theme.bg_focus      = wal.c02
theme.bg_urgent     = "#d53838"
theme.bg_minimize   = "#666666"
theme.bg_systray    = wal.c01 .. theme.transparency
...
theme.taglist_squares_unsel = meme_squares_unsel(
    taglist_square_size, theme.fg_normal
)
