...
local function bookmarkGet()
    ...
    if ( string.match(file, "watch%?v=.*") ) then
        file = "https://www.youtube.com/" .. file .. "&t=" .. math.floor(time_pos)
    end
    local bookmark = string.format("%s\n%02d:%02d:%02d.%03d\n%s\nClaim: \nComment: \n\n", name, hours, minutes, seconds, milliseconds, file)
    local log = io.open("/home/alex/vimwiki/Video Bookmarks.wiki", "a")
    if log:write(bookmark) then
        mp.osd_message(string.format("Bookmark: %s", name))
    else
        mp.osd_message("Failed to write bookmark")
    end
    log:close()
end

mp.add_key_binding("b", "bookmarkGet", bookmarkGet)
...
-- end
-- 
-- mp.add_key_binding("Ctrl+b", "copyTime", copyTime)
